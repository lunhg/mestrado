...
\begin{figure}[!h]
  \centering
  \includegraphics[scale=0.4]{imagens/screenbashing.png}
...
\begin{frame}{NI-Akoustik Piano}
\pressingthree{R}{ask}{1}: \emph{A Study in Keith} $\rightarrow$ \emph{Disklavier Sessions} \cite{sorensen_disklavier_2013}.
\end{frame}
...
\begin{frame}{Scheme}
Notação chamada \emph{notação prefixada}, inventada por Jan Łukasiewicz em 1924, para simplificar a lógica proposicional.

\lisptres
\end{frame}

\defverbatim{\lispquatro}{
\begin{minted}[fontsize=\scriptsize]{cl}
;; define A = 1
(define A 1)
...
\section{Analise}\label{sec:define_instr}

\newcommand{\tempo}[2]{$#1'~#2''$}

\defverbatim{\sorensenum}{
...
\defverbatim{\sorensendois}{
  \begin{minted}[fontsize=\footnotesize]{cl}
    (define *metro* (make-metro 110))
  \end{minted}
  }
 
\defverbatim{\sorensentres}{
  \begin{minted}[fontsize=\footnotesize]{cl}
...
\defverbatim{\sorensenquatro}{
\begin{minted}[fontsize=\footnotesize]{cl}
(pc:make-chord 50 70 2 (pc:diatonic 0 '- degree))
  \end{minted}
}

\begin{frame}{Analise: Definição de instrumento}
...
\begin{frame}{Definição de uma função auxiliar}
\sorensendois
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}
Um conjunto de características é definido pelo momento de execução do evento,\pressingthree{F}{ask}{0}, o grau, \pressingthree{F}{ask}{1}, e a duração deste evento, \pressingthree{F}{ask}{2}. 
\end{frame}

\begin{frame}{Primeira sequência de blocos de eventos}
O primeiro bloco de eventos \pressingthree{E}{ask}{0} aprensenta um contraponto de primeira espécie, aticulado em tempos fortes e fracos, de acordo com um movimento cadencial $i~\Rightarrow~vii$.
\end{frame}

\begin{frame}{Primeira sequência de blocos de eventos}
Uma característica \pressingthree{F}{ask}{0} do algoritmo é sua direcionalidade em um âmbito de quinta em um número de compassos pares (4 nesse caso). 
\end{frame}
...
\begin{frame}{Terceiro Bloco}
\pressingthree{E}{ask}{0} e \pressingthree{E}{ask}{1} $\rightarrow$  Sorensen realiza uma transformação na estratégia transversal
\end{frame}

\defverbatim{\sorensenoito}{
\begin{minted}[fontsize=\footnotesize]{cl}
    (define chords
       (lambda (time degree dur)
          (if (member degree '(i)) (set! dur 3.0))
...
\begin{frame}[allowframebreaks]{Primeira transformação da estratégia transversal}
\sorensenoito
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}[allowframebreaks]{Primeira transformação da estratégia transversal}
Transformação na acentuação 
\end{frame}

\begin{frame}{Transcrição do terceiro bloco}
\begin{figure}
  \centering
  \input{./ask5}
...
\begin{frame}{Futuro?}
\pressingthree{K}{ask}{1} $\rightarrow$ excede um objetivo deste documento.
\end{frame}
...
\begin{frame}{Conclusão}
  \emph{A Study in Keith} de Andrew \citeonline{sorensen_keith_2009} ( (\url{https://vimeo.com/2433947})
