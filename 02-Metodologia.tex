...
O \emph{Quadro Conceitual de Sistemas Criativos} \ver{sec:csf} foi utilizado como método de análise para uma improvisação de códigos \ver{cap:estudos_de_caso}.  Em termos cognitivos, este trabalho segue a aplicação de \emph{imagens mentais} \ver{sec:imagem_mental} e \emph{espaços conceituais}\ver{sec:espaco_conceitual}, cuja abordagem descreve um modelo computacional para o laço cognitivo entre o que é feito, observado, e transformado.
...
Em ambos os casos, na prática reflexiva e na bricolagem, são necessários conceitos que suportem a intenção (e vontade) do ofício. Isso quer dizer que conceitos são elaborados através de restrições de idéias, que  podem ser construídos com o auxílio de esquemas visuais. Esta abordagem é muito semelhante à posição defendida por McLean, onde conceitos podem ser representados geometricamente. No caso do(a) improvisador(a)-programador(a), \emph{espaços conceituais} são contínuamente reelaborados, começando por uma \emph{estratégia transversal}, passando para a observação, reação, e reformulação da imagem mental.
...
\section{Imagem Mental do improvisador-programador}\label{sec:imagem_mental}

Definir verbalmente, ``imagem mental'', em termos visuais, estabelece uma relação entre a observação e a verbalização. Por observação não queremos dizer apenas a visão, mas estados conduzidos por órgãos perceptivos. Para \citeonline[p.~24]{McLean2011}, imagens mentais se relacionam com quaisquer estados quase-perceptuais: \traducao{Por exemplo o uso de entonação prosódica na fala é \emph{paralinguística}, não inteiramente notada em texto escrito, mas ainda assim simboliza um conteúdo significativo.}{For example the use of prosodic intonation in speech is \emph{paralinguistic}, not entirely notated in written text, yet symbolising meaningful content}. Isto é, na teoria da \emph{Codificação Dual} \apud[p.~25--29]{paivio_dual_1990}{McLean2011}, é estabelecida uma relação de colaboração entre hierarquias de linguagem e códigos de percepção. Esta colaboração estrutura a informação comunicada: \traducao{humanos são capazes de compreender linguagem enquanto simultaneamente atendem às figuras $[$de linguagem$]$.}{humans are able to comprehend language while simultaneously attending to imagery.}:
...
A segunda hierarquia é descrita através do \emph{Quadro de estruturação das Dimensões Cognitivas da Notação} \apud[p.~95--97]{church_cognitive_2008}{McLean2011}. No caso, é um referencial para a elaboração de novas linguagens, cuja sintaxe pode ser moldada para  atividades específicas (\traducao{Linguagens de Domínio Específico}{DomainSpecific Languages } ou DSL). Para McLean, DSLs ``provêm termos padronizados para descrever demandas particulares em um domínio de uma tarefa''.\ver{tab:dimensoes}.
...
Não explicaremos todas, pois não são dimensões autônomas, o que diverge do foco de nosso trabalho. McLean exemplifica a viscosidade: a possibilidade de diferentes soluções para o mesmo resultado. Isto é, se uma Notação secundária é possível. Por exemplo, em um \emph{patch} de PD, cuja imagem mental é uma onda quadrada. Podemos utilizar objetos nativos ou objetos extendidos \ver{fig:pd}. 
...
\end{equation}\label{eq:vazio}
 ...
 Wiggins define o Universo de Conceitos (\csf{U}{x}) como um conjunto não estrito dos \emph{Espaços Conceituais} (\csf{E}{x}) de Margaret \citeonline{boden_creative_1990}. Isto é, um Universo de Conceitos a respeito de alguma coisa, no nosso caso da improvisação de códigos \ver{eq:ul}. 

\begin{equation}
\mathcal{U}_\emph{livecoding} = [\mathcal{E}_\emph{Tecelagem}, \mathcal{E}_\emph{Audiovisual}, \mathcal{E}_\emph{Dança} \mathcal{E}_\emph{Música}, \ldots, ?]
...
\section{Espaços conceituais e o processo de bricolagem}\label{sec:espaco_conceitual}

Vamos retornar à \autoref{fig:processo_criativo}. Três processos-chave são considerados: a primeira é a formação de um espaço de conceitos diante de imagem mental que o improvisador-programador experenciou. 
