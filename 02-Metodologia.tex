...
# Gera classificacoes gramticais
...
\end{citacao}
 ...
 Por último, a zona representa a constante transformação de idéias.
\begin{figure}[!h]
  \centering
  \includegraphics[scale=0.5]{imagens/rede1.png}
  \caption{Verticalização de uma rede de conceitos. \textbf{Fonte}: \citeonline{thornton_quantitative_2007}.}
  \label{fig:rede1}
\end{figure}

Esta composição é destacada na frase `` Pode ser empregada em muitos contextos diferentes de performance: dança, música, imagens em movimento e mesmo tecelagem.'' \ver{cap:introducao}. A partir da concatenação de diversos AND é possível tecer uma rede de conceitos para improvisação de códigos. No entanto, esta hierarquia verticalizada aumenta em complexidade conforme o número de relações aumenta, definido por \citeonline[p.~15-16]{thornton_quantitative_2007} como um conceito de complexidade conceitual bruta: \traducao{(\ldots) qualquer conceito $[$cujo$]$ o número de conceitos que deve ser desenvolvido a partir da fundação dada para garantir a sua construção}{The gross conceptual complexity of any concept is the number of concepts which must be developed from the given foundation to guarantee its construction} \label{fig:rede3}. Conceitos também podem ter extensões em um espaço conceitual, como um subgrupo que surge de uma nova verticalização, de maneira oposta, para encontrar, em um campo complexo de conceitos brutos, em conceitos úteis para a formação do espaço conceitual mínimo \ver{fig:rede4}. No entanto, como pontua \citeonline[p.~17]{thornton_quantitative_2007}, este procedimento de organização por redes de conceitos leva a uma \traducao{(\ldots) exaustiva enumeração de todas possibilidades construídas, podemos presumir que a maioria de tais conceitos são propensos a ter pouco valor}{(\ldots)exhaustive enumeration of all construction possibilities, it can be assumed that most such concepts are likely to have little value.}. No entanto, \citeonline[p.~520--521]{Forth2010} pontuam que essa concepção de uma desvalorização da abstração do conceito, em relação à rede de conceitos formada, ganha outras valorizações. Por exemplo, ao discutir a necessidade de um \emph{espaço conceitual mínimo} para estruturas de ritmos em uma linguagem de programaç\~ao \ver{sec:tidal}, o seguinte ponto de vista e colocado. como pontua  \citeonline[p.~17]{thornton_quantitative_2007}, ao sugerir observar a organização do espaço conceitual mínimo como a interesecção de regiões que se transformam o tempo inteiro, ou ``zonas esponjosas'' \ver{fig:rede4}.

\begin{figure}[!h]
  \centering
  \includegraphics[scale=0.35]{imagens/rede2.png}
  \includegraphics[scale=0.5]{imagens/rede3.png}
  \includegraphics[scale=0.5]{imagens/rede4.png}
  \includegraphics[scale=0.35]{imagens/rede5.png}
...
\begin{figure}[!h]
  \centering
  \includegraphics[scale=0.5]{imagens/rede4.png}
  \caption{ \textbf{Fonte}: \cite{thornton_quantitative_2007}}
  \label{fig:rede4}
\end{figure}
