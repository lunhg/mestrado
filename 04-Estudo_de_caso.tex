...
A análise de \emph{A Study in Keith} será feita com base em um recorte do \emph{Quadro Conceitual de Sistemas Criativos}\footnote{Creative System Frameworks, \emph{ou CSF}, \cfcite{mclean_music_2006,Forth2010,McLean2011}.} \ver{tab:universodeconceitos}. Este quadro se baseia no modelo de improvisação de Jeff \citeonline{pressing_improvisation_1987} \ver{sec:im}. É importante esclarecer que, o que os autores ingleses chamam de \emph{conceito}, entendemos ao longo deste trabalho como proposição para não confundirmos questões ontológicas. Desta forma analisamos uma única proposta de ação indicada por Sorensen \ver{sec:proposicao}
...
São duas as descrições de uma mesma proposição, ou \emph{Espaço conceitual de A Study in Keith} (\csf{E}{ask}). Andrew Sorensen cita também os Concertos \emph{Sun Bear} de Keith Jarret \ver{sec:sunbear} como inspiradores da improvisação de códigos. Desta forma, estes conceitos são um \emph{Referencial Zero de A Study in Keith}, ou \pressingthree{R}{ask}{0}.
...
\citeonline{sorensen_youtube_2014} indica outros referenciais, que chamamos de \emph{Referencial Um e Dois de A Study in Keith} \ver{sec:impromptu}, ou o ambiente de programação \emph{Impromptu} (\pressingthree{R}{ask}{1}) e a linguagem de programação \emph{Scheme} (\pressingthree{R}{ask}{2}):
...
O \emph{gospel} e o \emph{folk} são categorizados como gêneros musicais nesta suíte que não possui pausas entre as partes (o improviso é contínuo, mas seccionado por transições). Seu motivo gerador é, segundo Uwe \citeonline{karcher2009}, o tema de \emph{A song of the heart}:
...
A questão se Jarret ou não improvisou este tema cria uma outra pergunta: o código elaborado por \citeonline{sorensen_keith_2009}, e sua sonoridade resultante, é de fato uma improvisação de códigos, ou existe um agenciamento onde o improvisador prepara um código, e define um objetivo inicial?
...
Esta subseção é dedicada a explicar a \emph{Linguagem de A Study in Keith} (\csf{L}{ask}) \ver{tab:universodeconceitos}, ou a linguagem de programação \emph{Scheme}.
...
Átomos, extraídos de sentenças abstratas, como listas de átomos \citeonline[seção~3]{mccarthy_recursive_1960}:
...
Uma característica da \emph{Linguagem de A Study in Keith} (\csf{L}{ask}) é sua notação para expressar uma proposição como  ``some uma unidade e uma unidade, logo teremos duas unidades'',  ou $1+1=2$, para qualquer operação indefinida, como ``some elementos, que neste caso, são dois unitários, logo duas unidades'', ou $+ 1 1$:
...
Para os propósitos deste trabalho, será útil apresentar um código musical fictício, mais especificamente, direcionado como uma experiência mental de uma sonoridade de \emph{jazz} tonal \citeonline[p.~823-824]{sorensen_impromptu_2010}. 
...
Este código será discutido como uma estratégia transversal \csf{T}{ask}. Para isso, particionamos o código para uma melhor compreensão:
