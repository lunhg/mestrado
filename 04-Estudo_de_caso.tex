...
técnica musical, as improvisações de códigos resultam, em sua maioria, em sons processados eletronicamente. Esta característica não restringe o tipo de música, de forma que dois tipos podem ser destacados: a Música Eletrônica para Dançar e, de outro, a Música-Ruído (\emph{noise-music}) e Música-Erro (\emph{glitch-music}), ambas com suas diversas variantes \footnote{\cfcite{lunhani_alguns_2015}.}. 

Como buscamos argumentar nos dois primeiros capítulos, é característica de um problema geral apontado por \citeonline{soares_luteria_2015}:

Em seu livro sobre música e mediação tecnológica, Fernando Iazzetta (2009, p.
187) nos fala sobre um tipo de "luteria composicional" que surge do experimento de
estúdio e migra para os computadores pessoais no final do século XX. A criação dos instru-
mentos musicais a partir de algoritmos, códigos, patches 1 , passa a existir como estratégia
composicional, tanto quanto a técnica instrumental ou a experiência de orquestração por
escrita partitural.

surge com compositores não cabe em um processo de análise musical. Podemos analisar uma improvisação de acordo com uma estética. Mas em outra, ela será inválida.
 
Desta forma, nossa intenção não é analisar a técnica ou a teoria musical, e sim uma proposição, e sua \emph{estratégia transversal} \cite{mclean_music_2006,Forth2010,McLean2011}. Em outras palavras, o que foi imaginado e o significado de um algoritmo musical. 

Deste ponto de vista, um resultado tonal facilita o entendimento deste tipo de análise. Por outro lado, obtivemos mais informações de uma proposição de \emph{A Study in Keith} de Andrew Sorensen (2009), do que qualquer outra improvisação observada neste documento.

Neste sentido, este capítulo segue o instanciamento feito por \citeonline[p.~117]{McLean2011},

\begin{citacao}
\traducao{Para fundamentar a discussão em música, considere uma peça de \emph{jazz}, onde \emph{jazz} é um conceito e uma composição particular é uma instância de um conceito. O musicista, explorando os limites do \emph{jazz}, encontra então uma peça para além das regras usuais do \emph{jazz}. Através deste processo, os limites do gênero musical podem ser redefinidos em algum grau, ou se a peça está em um novo terreno particularmente fértil, um novo sub-gênero de \emph{jazz} emerge. Contudo uma peça de música que não quebra limites, de alguma forma pode ser considerada não-criativa}{
...
O contexto desta análise segue o \emph{Quadro Conceitual de Sistemas Criativos}\footnote{Creative System Frameworks, \emph{ou CSF}, \cfcite{mclean_music_2006,Forth2010,McLean2011}.}. Este quadro é construido com base no modelo de improsisação de Jeff \citeonline{pressing_improvisation_1987}.

É importante entender que, o que os autores ingleses chamam de conceito, entendemos como proposição. Desta forma traduzimos CSF como \emph{quadro de um sistema de proposições}, ou \emph{quadro de proposições}.

 Este quadro de proposições realiza uma discretização dos processos criativos de um artista-programador, o que possibilita discutir um modelo de estruturação, e análise, de improvisações de códigos musicais.

\input{04-Metodologia}

\section{A Study in Keith}
