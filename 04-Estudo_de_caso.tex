...
Uma abordagem cognitivista, como propõe \citeonline{McLean2011}, estabelece a leitura de ciclos de ação e reação, através da programação por bricolagem \ver{fig:bricolagem}. Neste trabalho, não vamos além do primeiro ciclo de elaboração-codificação (\emph{elaborate-encode}). Assim indicamos apenas um método de análise de uma parte da  \emph{estratégia transversal}. 
...
%A análise segue o \emph{Quadro Conceitual de Sistemas Criativos}\footnote{Creative System Frameworks, \emph{ou CSF}, \cfcite{mclean_music_2006,Forth2010,McLean2011}.}. Este quadro é construido com base no modelo de improsisação de Jeff \citeonline{pressing_improvisation_1987}. É importante esclarecer que, o que os autores ingleses chamam de conceito, entendemos como proposição. Desta forma trabalhamos como um \emph{quadro de um sistema de proposições}. 
...
%  %\includegraphics[scale=0.5]{imagens/Jarret_intro.png}
  \input{./Jarret2}
  \caption{Transcrição do motivo gerador do disco Kyoto, parte 1. textbf{Fonte}: autor.}
...
Uma pequeno esquema expoẽ o padrão melódico derivado de cromatismo (ver exemplo \ref{ex:schenker}, como \^1\^b2-\^1-\^7b, inicialmente em uma forma permutada, \^7-\^{b2}-\^1\footnote{Originalmente \^1\^b2-\^1-\^7, \cfcite[p.~4,p~.40,p~48,p.~74]{karcher2009}.}, como uma figura alternada com um ostinato no baixo, cuja primeira sonoridade, não possui uma função harmônica \emph{a priori}.
