\chapter{Definições de base sobre computadores e Improvisação de códigos}\label{cap:introducao}
...
\section{Weavecoding}\label{sec:tecelagem}
...
Em 2004, com a formação do TOPLAP \ver{sec:toplap}, Ward e McLean focaram seus eforços no desenvolvimento de ambientes de improvisação de códigos. É interessante notar que os sistemas elaborados,  são uma mistura de linguagens textuais, \emph{patching} e interfaces gráficas de usuário\footnote{\emph{Graphical User Interfaces} ou GUIs.}: \traducao{O \emph{Slub} controlava sua música usando interfaces criadas por e para eles mesmos. Eles variam desde as $[$interfaces$]$ aparentemente convencionais para as abstratas, e das  gráficas para as inteiramente textuais.}{Slub control their music using user interfaces created by and for themselves. These vary from the apparently conventional to the abstract, and from graphical to entirely textual.}
\cite[p.~323]{collins_generative_2003}. A descrição abaixo detalha algumas das funções destes programas, bem como um processo de composição por redes que será discutido em outra oportunidade \ver{sec:baiasaofranscisco}:
...
O \emph{algorave} é, em um sentido, essencial para o entendimento de categorizações musicais \footnote{\cfcite{janotti_jr._a_2003,sa_musica_2006,sa_se_2009}.} de um grupo social. Este grupo social, que com seus diferentes/semelhantes sujeitos,  cria artefatos artísticos frutos das relações entre as pessoas, o que por outro lado, possibilita criar institucionalizações dos artefatos. Discutiremos uma dessas institucionalizações, bem como sujeitos sociais influentes neste contexto, de forma que sua liminaridade\footnote{\cfcite{prospero_social_2015}} transita entre a Academia, salas de concertos e casas noturnas \ver{sec:showusyourscreens}. Tal institucionalização é chamada de TOPLAP \ver{sec:laptoptoplap}, e para descrevê-la, como uma construção social que possui ideologias, apresentamos algumas figuras influentes, não todas (pois não caberia no escopo de nosso trabalho), no cenário europeu. 

Sobre o termo \emph{algorave}, é interessante notar um comportamento criativo  por combinação \ver{sec:criatividade}. A compositora colombiana Alexandra Cárdenas, em entrevista com \citeonline{chesire_algorave_2013}, aponta que Nick Collins e Alex McLean combinaram dois termos \emph{algorithm}, e \emph{rave}, durante uma \emph{gig} (um termo utilizado no início do \emph{jazz} para caracterizar um trabalho temporário). Após sintonizarem em uma estação de rádio, decidiram combinar a música transmitida com a programação de uma música semelhante:
...
Em seu artigo ``Algorave: Live Performance of Algorithmic Electronic Dance Music'', \citeonline[p.~356]{collins_algorave_2014} sustentam que as estruturas das práticas do \emph{algorave} são anteriores à improvisação de códigos, utilizadas por praticantes da Música Eletrônica para Dançar. O que relaciona o \emph{algorave} e a improvisação de códigos é a prática de projeção do código \ver{sec:laptoptoplap}:
...
\section{Coreografia}\label{sec:coreografia}
...
Enumeramos três características que permearam as atividades do grupo, como FooBarBaz, e que ainda permeiam o contexto do \emph{labMacambira}: \emph{i}) abordagem ecossistêmica, ou diversidade de \emph{softwares}; \emph{ii}) atividade de escrita o mais simples possível; \emph{iii}) codificação como processo psicosocial -- humor como integrador de pessoas, como a inclusão de figuras afetivas, como um programa, \emph{cowsay} (``uma vaca falante configurável''\disponivelem{http://www.nog.net/~tony/warez/}), e de fotos de vacas se lambendo, e REM\footnote{\emph{Rapid Eye Movement}, ou uma fase do sono} com o movimento repetitivo de uma imagem  \ver{fig:foobarbaz}. 
...
\begin{example}{Resultado da compilação}
\begin{minted}[fontsize=\scriptsize]{bash}
...
Aqui admitimos um conflito ideológico (e espiritual) na análise do código. Apesar de compartilhar dos conceitos de agenciamento composicional, não compartilhamos de um significado inerente ao código. Neste sentido, o autor busca se defender da seguinte crítica feita na defesa deste documento: levar a piada a sério demais. 

Tomando um erro de tipos numéricos, como erro lógico, demonstramos a piada como recurso ideológico inferior \cite[p.~]{McLean2011}, ainda que essa seja o \emph{modus operandi} da improvisação de códigos. Nesse sentido, sugerimos seguir a cartilha científica, e propor uma interessante modificação no algoritmo de Magno. Os novos caracteres são relativos àqueles inseridos por Caliman em 2$'$18$''$, trocando por uma escrita automática, no limite do possível, sem ideologias:
...
O resultado não retorna erros:

\includegraphics[scale=0.35]{./imagens/autoescrita.png}

\end{example}

\section{Telepresença e espaços virtuais}\label{sec:telepresenca}

Uma performance virtual de \emph{live coding} é aquela em que dois ou mais executantes, em endereços diferentes de uma rede de computadores. Isso situa três casos, que buscamos apenas categorizar: i) uma rede local, com computadores diferentes, mas com os improvisadores fisicamente próximos; ii) uma rede remota, privada, que comunica um conjunto de pessoas fisicamente distantes; iii) a rede mundial de computadores, onde o navegador se torna o ambiente virtual de criação musical \cite{roberts_web_2013}. Nos três casos, a premissa é compartilhar o mesmo código entre improvisadores-programadores. Podem também compartilhar do mesmo som, mas isso depende de implementações técnicas . 

O primeiro caso é bastante documentado nas \emph{live coding sessions}, sessões de improvisações que ocorrem em encontros, simpósios e \emph{workshops}, ricamente documentadas\disponivelem{https://supercollider.github.io/archive}. São encontros informais, onde programadores estão próximos uns aos outros. No \autoref{cap:protohistoria} (ver p.~\pageref{cap:protohistoria}), investigamos mais de perto bases históricas desta prática com os grupos \emph{The League of Automatic composers}/\emph{The Hub}.

 Dois exemplos do segundo caso são descritos por \citeonline[p.~152--153]{junior_supercopair_2015}.  Um trabalho original, \emph{Supercopair}\disponivelem{https://github.com/deusanyjunior/atom-supercopair}, possibilita a criação de um ambiente virtual para a improvisação de códigos. Através da computação em nuvem, dois usuários podem compartilhar códigos sem complicadas configurações \ver{fig:supercopair}. O segundo exemplo, similar, sem mas sem computação em nuvem.  Sua descrição pormenorizada é demasiadamente técnica para o escopo deste trabalho, e desta forma, apontamos artistas-programadores como Nick Collins, John Rohrhuber, Ben Swift e Andrew Sorensen. Os dois primeiros trabalham com redes TCP/IP, enquanto os dois últimos trabalham \traducao{entre dois intérpretes-programadores localizados na Alemanha e Estados Unidos usando um servidor SSH localizado na Australia}{between two live coders located in Germany and United States using an SSH server located in Australia.}. 

 \begin{figure}[!h]
   \centering
   \includegraphics[scale=0.5]{imagens/supercopair.png}
   \caption{Esquema prático do \emph{Supercopair}, um ambiente colaborativo para a improvisação de códigos com o \emph{SuperCollider} através da nuvem de servidores. \textbf{Fonte}: \citeonline[p.~55]{junior_supercopair_2015}.}
   \label{fig:supercopair}
 \end{figure}
